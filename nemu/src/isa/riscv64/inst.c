...
#define INSTPAT_MATCH(s, name, type, ... /* body */)             \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B, if ((signed)src1 < (signed)src2) s->dnpc = s->pc + dest);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti, I, R(dest) = ((signed)src1 < (signed)src2));
  ...
  // INSTPAT("000000 ?????? ????? 001 ????? 00100 11", slli   , I, R(dest) = (src1 << BITS(src2,5,0)));           // RV64I
  // INSTPAT("000000 ?????? ????? 101 ????? 00100 11", srli   , I, R(dest) = (src1 >> BITS(src2,5,0)));           // RV64I
  // INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((signed)src1 >> BITS(src2,5,0)));   // RV64I
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra, R, R(dest) = (signed)src1 >> BITS(src2, 5, 0));
  ...
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai, I, R(dest) = ((signed)src1 >> BITS(src2, 5, 0)));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw, I, R(dest) = SEXT(((signed)BITS(src1, 31, 0) >> BITS(src2, 4, 0)), 32));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(dest) = (uint64_t)(((__int128)(int64_t)src1 * (__int128)(int64_t)src2) >> XLEN));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(dest) = (uint64_t)(((__int128)(int64_t)src1 * (unsigned __int128)src2) >> XLEN));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, R(dest) = (uint64_t)(((unsigned __int128)src1 * (unsigned __int128)src2) >> XLEN));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div, R, R(dest) = (signed)src1 / (signed)src2);
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw, R, R(dest) = SEXT(((signed)SEXT(src1, 32) / (signed)SEXT(src2, 32)), 32));
  ...
  INSTPAT("???????????? ????? 001 ????? 11100 11", csrrw, I, word_t t = CSRs(src2); CSRs(src2) = src1; R(dest) = t);
  INSTPAT("???????????? ????? 011 ????? 11100 11", csrrc, I, word_t t = CSRs(src2); CSRs(src2) = t & ~src1; R(dest) = t);

  // Privileged
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret, R, s->dnpc = CSRs(0x341) + 4;);// some fun wait to add
